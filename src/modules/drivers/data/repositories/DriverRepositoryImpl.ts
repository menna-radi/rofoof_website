...
export class DriverRepositoryImpl implements IDriverRepository {
  ...
  async getDrivers(): Promise<DriverEntity[]> {
    if (ENV.USE_MOCK) return Promise.resolve(this.drivers);
    const res = await apiClient.get<DriverEntity[]>(ENDPOINTS.DRIVERS.LIST);
    return res.data;
  }

  async getDriverById(id: string): Promise<DriverEntity | null> {
    if (ENV.USE_MOCK) return Promise.resolve(this.drivers.find((d) => d.id === id) || null);
    const res = await apiClient.get<DriverEntity>(ENDPOINTS.DRIVERS.DETAIL(id));
    return res.data;
  }
  ...
  async createDriver(driver: Partial<DriverEntity>): Promise<DriverEntity> {
    ...
    const res = await apiClient.post<DriverEntity>(ENDPOINTS.DRIVERS.CREATE, driver);
    return res.data;
  }
